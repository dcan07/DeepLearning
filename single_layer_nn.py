...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  3 15:11:09 2020
...
#main loop
i=0
while(prev_obj-obj>stoppingcondition and i<epochs):
      #assign objective value
      prev_obj=obj
    
      #calculate gradients
      d0,d1=calculate_gradient(train_data,train_label,input_weights,hidden_weights)
...
for i in range(len(test_data)):
    predictions[i]=np.dot(sigmoid(np.matmul(test_data[i],input_weights.T)),hidden_weights)
#label based on sign
predictions=[1 if x>0 else -1 for x in predictions]

#print predictions error and weights
print('Prediction=',predictions)
print('Error=',1-accuracy_score(test_label,predictions))
print('w=',hidden_weights)
