...
#it has 51 columns
train=np.array(data.iloc[:,0:51])

#Testlabel set is the last week
#We only use it to calculate MSE at the end
testlabel=np.array(data.iloc[:,51])
...
for row in range(len(train)):
    ...
    #Get row of data, which is only train
    productdata=np.reshape(train[row,:],newshape=(51,1))
    #LSTM encoding
    for i in range(lookback_window, len(productdata)):
        x.append(productdata[i -lookback_window:i])
        y.append(productdata[i])
    #get x_train, x_test, y_train
    #we dont have y_test, it is the true value of last week
    #only used to calculate the mse
    x_train = np.array(x)
    y_train = np.array(y)
    x_test=productdata[(len(productdata)-lookback_window):].reshape((1,lookback_window,1))

    
    #TCN
    i = Input(shape=(lookback_window, 1))
    m = TCN(nb_filters=64,nb_stacks=2)(i)
    m = Dense(1, activation='linear')(m)
    
    model = Model(inputs=[i], outputs=[m])    
    #model.summary()  
    model.compile('adam', 'mse')   
    model.fit(x_train, y_train, epochs=numberofepochs, verbose=0)
    
    #Get predictions
    predictions[row]=model.predict(x_test)
    


#This is the first time we are using testlabels(Last week)    
print('Mean Squared Error is: ',mean_squared_error(testlabel,predictions))
